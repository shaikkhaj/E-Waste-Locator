...
document.getElementById("submit").addEventListener("click", () => {
  ...
  const company = document.getElementById("device-company").value;
  ...
  fetch("php/register.php", {
    ...
    body: `name=${name}&deviceType=${deviceType}&company=${company}&model=${model}&mobile=${mobile}`
  })
  .then(response => response.json())
  .then(data => {
    if (data.success) {
      document.getElementById("points-value").textContent = data.points;
      document.getElementById("reward-points").style.display = "block";
    } else {
